# MiterBox - BoxFxTix for Miter, a Graphical Ascii Form and Table Template Tool
...
module MiterBox
  ...
  module BoxFx
    ...
    def box_fx_cut
      ...
      @@label_font = Fx::FxFont.new('helvetica', 'size'=>12)
      
      @@button_font = Fx::FxFont.new('system')
      @@button_font.configure('size'=>12)

      @@data_font = Fx::FxFont.new('helvetica')
      @@data_font.configure('size'=>10)

      #
      root_container = Fx::FxRoot.new

      #
      main_frame = FxCanvas.new(root_container)
      
      main_scrollbar = FxScrollbar.new(root_container) {
        command proc { |*args|
      ...
      #
      table_container = box_fx_build_table(main_frame, @main)
    ...
    #
    def box_fx_build_table(parent, table, i=nil)
      
      table_container = Fx::FxFrame.new(parent) {
        bd DBD
        relief DBR
      }
        ...
        if section.repeat
          repeat_length(section).times do |j|
            section.cells.each do |cell|
              cell_container = box_fx_build_cell(table_container, cell, j)
              cell_container.grid('row'=>cell.row+j, 'column'=>cell.col, 'rowspan'=>cell.rowspan, 'columnspan'=>cell.colspan, 'sticky'=>'snew')
            end
          end
        else
          section.cells.each do |cell|
            cell_container = box_fx_build_cell(table_container, cell, i)
      ...
      # configure column sizes (well, as best as we are able since Fx sucks!)
      table.scale.each_with_index do |c, i|
        ...
        FxGrid.columnconfigure(table_container, i, 'weight'=>col_weight, 'minsize'=>col_minsize)
      end
    ...
    #
    def box_fx_build_cell(parent, cell, i=nil)
      
      cell_container = Fx::FxFrame.new(parent) {
        bd DBD
        relief DBR
      }
      ...
      cell.elements.each do |element|
        ...
        if table and element.type_name == 'general' and not @disable.include?(element.name)
          table_container = box_fx_build_table(cell_container, table, i)
          ...
          when 'label'
            @box_widgets[element.name] = box_fx_build_label(cell_container, element, cell, i)
          when 'text'
            if element.size == 1
              @box_widgets[element.name] = box_fx_build_entry(cell_container, element, i)
            else
              @box_widgets[element.name] = box_fx_build_text(cell_container, element, cell, i)
            end
          when 'select'
            raise "select list missing for #{element.name}" if not @cells["#{element.name}_"]
            if element.size == 1
              @box_widgets[element.name] = box_fx_build_optionmenu(cell_container, element, i)
            else
              @box_widgets[element.name] = box_fx_build_listbox(cell_container, element, i)
            end
          when 'button'
            @box_widgets[element.name] = box_fx_build_button(cell_container, element, i)
          when 'radio' # and checkbox
            if element.size == 1  # check box
              @box_widgets[element.name] = box_fx_build_checkbox(cell_container, element, i)
            else
              @box_widgets[element.name] = box_fx_build_radio(cell_container, element, i)
            end
          when 'counter'
            @box_widgets[element.name] = box_fx_build_counter(cell_container, element, cell, i)
          else # general
            @box_widgets[element.name] = box_fx_build_general(cell_container, element, cell, i)
          end
          # pack it in
    ...
    #
    def box_fx_apply_attribute(aname, aclass)
    ...
    # GENERAL
    def box_fx_build_general(parent, element, cell, i)
    ...
    # LABEL
    def box_fx_build_label(parent, element, cell, i)
    ...
    # COUNTER
    def box_fx_build_counter(parent, element, cell, i)
      
      if element.name == 'index'
        counter = Fx::FxLabel.new(cell_container) { 
          text "#{i}"
          font @@label_font
          anchor ALIGN2ANCHOR[cell.alignment]
        }
    ...
    # ENTRY
    def box_fx_build_entry(parent, element, i)
      ...
      # build text entry
      textentry = Fx::FxEntry.new(parent) {
        background 'white'
        font @@data_font
      }
    ...
    # TEXT
    def box_fx_build_text(parent, element, cell, i)
      ...
      # build text entry
      textentry = Fx::FxText.new(parent) {
        height element.size
        background 'white'
        setgrid true
        wrap 'none'
    ...
    # LISTBOX
    def box_fx_build_listbox(parent, element, i)
      ...
      # build listbox
      listbox = Fx::FxListbox.new(parent) {
        height element.size
        background 'white'
        font @@data_font
      }
    ...
    # OPTIONMENU
    def box_fx_build_optionmenu(parent, element, i)
      ...
      # make bound variable
      bvar = Fx::FxVariable.new
      bvar.value = value
      
      # build dropmenu
      menubutton = Fx::FxMenubutton.new(parent) {
        textvariable bvar
        indicatoron 'on'
        relief 'sunken'
        borderwidth 2
    ...
    # BUTTON
    def box_fx_build_button(parent, element, i)
    ...
    # CHECKBOX
    def box_fx_build_checkbox(parent, element, i)
      ...
      checkbox = Fx::FxCheckbutton.new(parent) {
        variable bvar
        onvalue i ? i.to_s : '1'
        offvalue ''
      }
    ...
    # RADIO
    def box_fx_build_radio(parent, element, i)
      ...
      # build radio button
      radio = Fx::FxRadiobutton.new(parent) {
      }
