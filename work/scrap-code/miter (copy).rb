...
module MiterBox
  ...
  class Miter
    ...
    #
    def lay(layout_url, cells, prefix='', disable=[])
      @layout_path = File.dirname(layout_url)
      @layout = fetch_url(layout_url)
      @cells = cells
      @disable = disable
      @tables = []
    ...
    def external_sub(layout)
      layout.gsub!(/&.+$/) do |match|
        sublayout_url = match[1..-1].strip
        if is_relative?(sublayout_url)
          sublayout_url = "#{@layout_path}/#{sublayout_url}"
        end
        external_sub(fetch_url(sublayout_url) << "\n")
      end
      return layout
    end
